...
export function useBookmarkStorage() {
  ...
  const getChapter = (mangaTitle?: string, chapterId?: string | number): Chapter | null => {
    if (!mangaTitle || chapterId === undefined) return null
    const manga = getManga(mangaTitle)
    if (!manga) return null
    const numChapterId = typeof chapterId === 'string' ? parseInt(chapterId) : chapterId
  ...
  const isChapterFinished = (mangaTitle?: string, chapterId?: string | number): boolean => {
    const chapter = getChapter(mangaTitle, chapterId)
    return chapter?.isFinished || false
  }

  const getCurrentPage = (mangaTitle?: string, chapterId?: string | number): number => {
    const chapter = getChapter(mangaTitle, chapterId)
