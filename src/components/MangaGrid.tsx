...
const MangaGrid = ({
  ...
  useEffect(() => {
    ...
    const fetchMangas = async () => {
      try {
        ...
        if (isMounted) {
          setError(
            err instanceof Error ? err.message : "Failed to load mangas",
          );
        }
      } finally {
        if (isMounted) {
  ...
  const displayedMangas = useMemo(() => {
    return showAll
      ? filteredMangas
      : filteredMangas.slice(0, DEFAULT_DISPLAY_LIMIT);
      ...
      <section className='mb-12'>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6'>
          {Array.from({ length: SKELETON_COUNT }).map((_, i) => (
            <div
              key={i}
              className='bg-muted animate-pulse rounded-lg aspect-[3/4]'
            />
          ))}
        </div>
      </section>
  ...
  const showViewAllButton =
    !showAll && filteredMangas.length > DEFAULT_DISPLAY_LIMIT;
      ...
      <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6'>
        {displayedMangas.map((manga) => (
          <MangaCard
            ...
            rating={4.5}
            chapters={[]}
            status={enumStatus(manga.status) || "En cours"}
