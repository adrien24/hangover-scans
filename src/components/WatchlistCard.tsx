...
const WatchlistCard = ({ 
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "Reading": return "bg-green-600 hover:bg-green-600";
      case "Completed": return "bg-blue-600 hover:bg-blue-600";
      case "Plan to Read": return "bg-yellow-600 hover:bg-yellow-600";
      case "On Hold": return "bg-gray-600 hover:bg-gray-600";
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "Reading": return <Play className="w-3 h-3" />;
      case "Completed": return <CheckCircle className="w-3 h-3" />;
      case "Plan to Read": return <Clock className="w-3 h-3" />;
    ...
    <Card className="group relative overflow-hidden bg-manga-card border-border hover:bg-manga-card-hover transition-all duration-300">
      <div className="flex">
        ...
        <div className="relative w-24 sm:w-32 aspect-[3/4] overflow-hidden flex-shrink-0">
          ...
          {/* Status badge */}
          <Badge 
            className={`absolute top-2 left-2 text-white text-xs flex items-center gap-1 ${getStatusColor(status)}`}
          >
        ...
        <div className="flex-1 p-4 flex flex-col justify-between">
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {status === "Plan to Read" ? "Start Reading" : "Continue"}
            </Button>
          </div>
        </div>
      </div>
